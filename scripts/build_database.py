...
def download_real_images():
    ...
    for filename, url in REAL_IMAGE_SOURCES.items():
        ...
        # 파일이 이미 정상적으로 크게 저장되어 있다면 다운로드 패스
        if os.path.exists(dest_path) and os.path.getsize(dest_path) > 3000:
            print(f"[-] 이미 존재함: {filename} (로컬 캐시 사용)")
            continue
...
TEMPLATES = [
    ...
    # 3. 남북국 시대 (16~20번)
    {
        ...
        "answer": 2,
        "explanation": "해당 국가는 발해입니다. 발해는 당의 3성 6부제를 수용하되 정당성 하부에 충·인·의·지·예·신 6부를 독자적인 명칭과 좌우사 이원 집행 방식으로 운영했습니다. 제시자료는 대동여지도 실제 고서 지도 사진입니다.",
