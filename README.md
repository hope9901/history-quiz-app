...
- 문항은 국사편찬위원회 주관 **한국사능력검정시험 22개 회차(제57~78회 심화, 총 1,100문항)** 의 [공식 홈페이지 시험자료실](https://www.historyexam.go.kr/pst/list.do?bbs=dat)에서 공개한 문제지·정답표 원문을 문항 단위 이미지로 잘라 **변형 없이** 수록한 것입니다.
...
- 데이터 빌드 파이프라인: `scripts/build_from_official_pdfs.py`
  ...
  3. 텍스트 레이어가 없는 스캔 PDF 회차는 Windows 내장 OCR(`scripts/ocr_words.ps1`, 한국어 언어팩 필요)로 문항 좌표를 감지
  4. 문항별 시대(선사~현대)는 문항 텍스트의 키워드 점수로 자동 추정 (미분류는 시대순 배열 특성을 이용해 인접 문항으로 보간)
